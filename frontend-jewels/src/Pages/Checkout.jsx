...
const Checkout = () => {
  ...
  const grandTotal = cartItems.reduce(
    (acc, item) => acc + item.quantity * (item.variant?.totalPrice || 0),
    0
  );
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      // Rename `orderItems` to `cartItems` to match backend
      ...
      const payload = {
        userId: user?.id || "",
        shippingInfo,
        cartItems: formattedCartItems, // ✅ matches backend now
        paymentMethod: "COD", // optional
      };

      console.log("Order Payload :", payload);

      const res = await axios.post(`${apiurl}/order/place-order`, payload, {
        withCredentials: true,
      });

      if (res.data.success) {
        console.log("✅ Order Placed Successfully:", res.data.orderId);
        setOrderPlaced(true);
        localStorage.removeItem("cart");
        navigate("/account", { state: { defaultTab: "orderHistory" } });
      } else {
              ...
              {cartItems.map((item) => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <p className="marron montserrat font-semibold">
                      ₹{item.quantity * item.variant?.totalPrice}
                    </p>
                  </div>
                </div>
              ))}
